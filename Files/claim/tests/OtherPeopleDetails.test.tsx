...
const { OtherPersonDetails } = jest.requireMock('~/feature/claim/car/components/dumb');
const { Question, AddDetailsOrSkip } = jest.requireMock('~/feature/claim/shared/components');
...
describe('OtherPeopleDetailsComponent', () => {
  ...
  beforeEach(() => {
    ...
    (useAppSelector as jest.Mock).mockImplementation(selector => selectorValues.get(selector));
    ...
    (thunks.addOtherPerson as jest.Mock).mockReturnValue({ type: 'add-other-person' });
    (thunks.updateOtherPeople as jest.Mock).mockReturnValue({ type: 'update-other-people' });
    (sharedThunks.resetModelState as jest.Mock).mockReturnValue({ type: 'reset-model-state' });
  });
  ...
  it('should add an other person when Yes is clicked', () => {
    ...
    expect(thunks.addOtherPerson).toHaveBeenCalledWith(`${modelPath}.otherPeopleDetails`);
  ...
  it('should reset the model and jump to the next question when No is clicked', () => {
    ...
    expect(sharedThunks.resetModelState).toHaveBeenCalledWith(`${modelPath}.otherPeopleDetails`);
    expect(dispatch).toHaveBeenCalledWith({ type: 'reset-model-state' });
    expect(jumpToNextQuestionEx).toHaveBeenCalledWith('#addOrSkipOtherPeopleDetails');
  });
  ...
  it('should update other people when adding or removing', () => {
    ...
    const { handleAdd, handleRemove } = MultiBlock.mock.calls[0][0];

    handleAdd(1);

    expect(thunks.updateOtherPeople).toHaveBeenCalledWith(1, true, otherPeople);

    handleRemove(1);

    expect(thunks.updateOtherPeople).toHaveBeenCalledWith(1, false, otherPeople);
    expect(dispatch).toHaveBeenCalledTimes(2);
    expect(dispatch).toHaveBeenNthCalledWith(1, { type: 'update-other-people' });
    expect(dispatch).toHaveBeenNthCalledWith(2, { type: 'update-other-people' });
  });
  ...
  it('should show the add link only for the last person before the maximum', () => {
    ...
    expect(multiBlockItems.map((item: { showAddLink: boolean }) => item.showAddLink)).toEqual([false, true]);
    expect(multiBlockItems.every((item: { showRemoveLink: boolean }) => item.showRemoveLink === true)).toBe(true);
  });
  ...
  it('should hide the add link when maximum count is reached', () => {
    const people = Array.from({ length: MAX_ALLOWED_OTHER_PEOPLE_COUNT }, (_, index) => ({
      address: { addressLine1: `${index + 1} Test Street` }
    }));

    selectorValues.set(selectors.getOtherPeopleDetails, people);
    selectorValues.set(selectors.getOtherPeopleDetailsCount, MAX_ALLOWED_OTHER_PEOPLE_COUNT);

    render(<OtherPeopleDetailsComponent />);

    const { multiBlockItems } = MultiBlock.mock.calls[0][0];

    expect(multiBlockItems[MAX_ALLOWED_OTHER_PEOPLE_COUNT - 1].showAddLink).toBe(false);
  });
});