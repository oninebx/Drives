...
import PreStep1, { PreStep1Component } from './PreStep1';
...
import { ClaimType } from '~/feature/claim/shared/state/constants';
...
jest.mock('react-redux-form', () => ({ Form: ({ children }: { children: React.ReactNode }) => <form>{children}</form> }));
jest.mock('~/common/utilities', () => ({ logError: jest.fn(), raiseFieldGAEvent: jest.fn() }));
jest.mock('~/common/state', () => ({
  routes: { CLAIM: { WINDSCREEN: { PAGE1: '/claim/windscreen/page1' }, SHARED: { PRE_STEP_2: '/claim/pre-step-2' } } }
}));
jest.mock('~/common/state/selectors', () => ({ getFlags: jest.fn() }));
jest.mock('~/root/store', () => ({ useAppDispatch: jest.fn(), useAppSelector: jest.fn() }));
jest.mock('~/feature/portal/hooks/useDeconstructedParams', () => ({ useDeconstructedParams: jest.fn() }));
jest.mock('~/feature/claim/utils', () => ({ raiseClaimGAEvent: jest.fn() }));
jest.mock('~/feature/claim/shared/state/constants', () => ({
  ClaimType: { Motor: 'Motor', Contents: 'Contents', House: 'House', Landlord: 'Landlord', Windscreen: 'Windscreen' },
  modelPath: 'claim'
}));
...
jest.mock('~/feature/claim/contents/components', () => ({
  ...
  NaturalDisasterCause: () => <div data-testid="contents-natural-disaster" />,
  StolenFrom: () => <div data-testid="contents-stolen-from" />,
  WhereLastRememberHavingItems: () => <div data-testid="contents-where-last-remembered" />
}));

jest.mock('~/feature/claim/house/components', () => ({
  Damage: () => <div data-testid="house-damage" />,
  FireCause: () => <div data-testid="house-fire-cause" />,
  NaturalDisasterCause: () => <div data-testid="house-natural-disaster" />
}));
...
jest.mock('~/feature/claim/car/components', () => ({
  ...
  BoatNaturalDisasterCause: () => <div data-testid="boat-natural-disaster" />,
  ...
  CarNotCoveredMessage: () => <div data-testid="car-not-covered" />,
  CarRecovered: () => <div data-testid="car-recovered" />,
  CauseOfLoss: () => <div data-testid="cause-of-loss" />,
  Damage: () => <div data-testid="motor-damage" />,
  DamagedWhileParkedCause: () => <div data-testid="damaged-while-parked" />,
  MultiVehicleAccident: () => <div data-testid="multi-vehicle-accident" />,
  NaturalDisasterCause: () => <div data-testid="motor-natural-disaster" />
}));

jest.mock('~/feature/claim/shared/components', () => ({
  CatastropheEvent: () => <div data-testid="catastrophe-event" />,
  CustomerPolicies: () => <div data-testid="customer-policies" />,
  EventDate: () => <div data-testid="event-date" />,
  EventTime: () => <div data-testid="event-time" />,
  FloatingToolbar: () => <div data-testid="floating-toolbar" />
}));

jest.mock('~/feature/claim/shared/components/dumb/FormFooter/FormFooter', () => ({
  __esModule: true,
  default: ({ handleSubmit, disabled }: { handleSubmit: () => void; disabled: boolean }) => (
    <button data-testid="form-footer" disabled={disabled} onClick={handleSubmit}>Next</button>
  )
}));

jest.mock('~/feature/claim/shared/components/smart/PotentialDuplicateClaim/PotentialDuplicateClaim', () => ({
  PotentialDuplicateClaim: () => <div data-testid="potential-duplicate-claim" />
}));

jest.mock('~/common/components/base', () => ({
  FormMessage: ({ id, children }: { id?: string; children?: React.ReactNode }) => <div data-testid={id || 'form-message'}>{children}</div>,
  Html: ({ rawHtml }: { rawHtml: string }) => <span>{rawHtml}</span>,
  LoaderButton: ({ id, onClick, disabled, children }: { id: string; onClick: () => void; disabled?: boolean; children?: React.ReactNode }) => (
    <button id={id} disabled={disabled} onClick={onClick}>{children}</button>
  ),
  Spinner: () => <div data-testid="spinner" />
}));

jest.mock('react-md/lib/FontIcons/FontIcon', () => ({ __esModule: true, default: ({ children }: { children?: React.ReactNode }) => <span>{children}</span> }));

jest.mock('@tower/tui', () => ({
  InformationBox: {
    Container: ({ children }: { children: React.ReactNode }) => <div data-testid="information-box">{children}</div>,
...
jest.mock('./styles', () => ({
  StyledList: ({ children }: { children: React.ReactNode }) => <ul>{children}</ul>,
  StyledListItem: ({ children }: { children: React.ReactNode }) => <li>{children}</li>,
  StyledLabel: ({ children }: { children: React.ReactNode }) => <label>{children}</label>,
  StyledCheckbox: ({ checked, onCheckedChange }: { checked: boolean; onCheckedChange: () => void }) => (
    <input type="checkbox" data-testid="water-damage-checkbox" checked={checked} onChange={onCheckedChange} />
  ),
  StyledLink: ({ children, onClick, ...props }: { children: React.ReactNode; onClick?: () => void }) => <a {...props} onClick={onClick}>{children}</a>
}));

jest.mock('./usePreStep1ViewModel', () => ({ usePreStep1ViewModel: jest.fn() }));

describe('PreStep1', () => {
  const dispatch = jest.fn();
  const claimSharedState = { claimNumber: 'CLAIM-123', claimType: ClaimType.Motor, loadingPolicies: false, loadingCatCodes: false, loadingClaims: false };
  ...
  beforeEach(() => {
    ...
    selectorValues.set(selectors.getPendingPolicies, []);
    selectorValues.set(selectors.isDamagedAndBreakInHouseOrLandlord, false);
    selectorValues.set(selectors.getActiveCatCodes, undefined);
    selectorValues.set(selectors.isCatEventAnswered, false);
    selectorValues.set(selectors.isPolicyNotShownSelected, false);
    selectorValues.set(selectors.isEISPolicySelected, true);
    selectorValues.set(selectors.showContentsDamage, false);
    selectorValues.set(selectors.showContentsStolenFrom, false);
    selectorValues.set(selectors.showContentsWhereLastRememberHavingItems, false);
    selectorValues.set(selectors.isCannotClaimOnline, false);
    selectorValues.set(selectors.showContinuePreClaim, true);
    selectorValues.set(selectors.policyDetailsLoaded, false);
    selectorValues.set(selectors.getSelectedCatEventStartDate, undefined);
    selectorValues.set(selectors.getSelectedCatEventEndDate, undefined);
    selectorValues.set(selectors.showDamage, false);
    selectorValues.set(selectors.showBoatDamage, false);
    selectorValues.set(selectors.showBoatImpact, false);
    selectorValues.set(selectors.showBoatTheft, false);
    selectorValues.set(selectors.showBoatTheftEntry, false);
    selectorValues.set(selectors.showBoatSubmersion, false);
    selectorValues.set(selectors.showBoatNaturalDisaster, false);
    selectorValues.set(selectors.showBoatFire, false);
    selectorValues.set(selectors.getSelectedCatEvent, undefined);
    selectorValues.set(selectors.showNaturalDisasterCause, false);
    selectorValues.set(selectors.showHouseDamage, false);
    selectorValues.set(selectors.showContentsNaturalDisasterCause, false);
    selectorValues.set(selectors.showHouseNaturalDisasterCause, false);
    selectorValues.set(selectors.isClaimPreStep1PageValid, true);
    selectorValues.set(selectors.showCarGlassOnlyDamage, false);
    selectorValues.set(selectors.hideCauseOfLossInCatEvent, false);
    selectorValues.set(selectors.disableContinuePreClaim, false);
    selectorValues.set(selectors.showMultiVehicleAccident, false);
    selectorValues.set(selectors.showDamagedWhileParkedCause, false);
    selectorValues.set(selectors.showCarRecovered, false);
    selectorValues.set(selectors.showContentsFireCause, false);
    selectorValues.set(selectors.showHouseFireCause, false);
    selectorValues.set(selectors.isHouseKeysClaim, false);
    selectorValues.set(selectors.isMotorKeysClaim, false);
    selectorValues.set(selectors.isCannotClaimForPolicyOnline, false);
    selectorValues.set(selectors.isWindscreenClaim, false);
    selectorValues.set(selectors.getPotentialDuplicateClaim, false);
    ...
    (thunks.handleStartProductClaim as jest.Mock).mockReturnValue({ type: 'start-product-claim' });
    (usePreStep1ViewModel as jest.Mock).mockReturnValue(defaultViewModel);
    (useDeconstructedParams as jest.Mock).mockReturnValue({ id: 'POLICY-123' });
  ...
  describe('PreStep1Loader', () => {
    ...
    it('should initialise the page on mount', () => {
      ...
      expect(dispatch).toHaveBeenCalledWith({ type: 'initialise-pre-step-1' });
    });
  });
  ...
  describe('catastrophe event', () => {
    it('should show catastrophe event when conditions are met', () => {
      selectorValues.set(selectors.getActiveCatCodes, ['CAT-1']);
      selectorValues.set(selectors.isEISPolicySelected, true);
      selectorValues.set(selectors.isPolicyNotShownSelected, false);
      selectorValues.set(selectors.isPolicyIneligibleForSSPClaimSelected, false);
  ...
  describe('house and landlord claims', () => {
    it.each([ClaimType.House, ClaimType.Landlord])('should render house components for %s claim', (claimType) => {
      selectorValues.set(selectors.getClaimSharedState, { ...claimSharedState, claimType });
      selectorValues.set(selectors.showHouseDamage, true);
