...
jest.mock('react-md', () => ({
  SelectionControlGroup: (props: any) => {
    ...
    return (
      <button
        data-testid="SelectionControlGroup"
        onClick={() => props.onChange('Other')}>
...
jest.mock('~/feature/claim/contents/state', () => ({
  ...
  selectors: {
    getNonSpecifiedItemDamageDetails: jest.fn()
  },
...
describe('ItemDetailsNonSpecifiedComponent', () => {
  ...
  describe('primary item type', () => {
    it('renders the primary type question', () => {
      ...
      expect(
        screen.getByTestId(
          'Question-questionItemDetailsNonSpecifiedPrimaryType-0'
        )
      ).toBeInTheDocument();
    });
    ...
    it('passes the correct props to SelectionControlGroup', () => {
      ...
      expect(mockSelectionControlGroup).toHaveBeenCalledWith(
    ...
    it('updates item type and raises GA event when primary type changes', () => {
      ...
      fireEvent.click(
        screen.getByTestId('SelectionControlGroup')
      );

      expect(
        mockUpdateNonSpecifiedItemType
      ...
      expect(raiseFieldGAEvent).toHaveBeenCalledWith(
        ...
        'radio',
        'itemDetailsNonSpecifiedPrimaryType',
        'Other'
      );
    });
  });
  ...
  describe('other item type', () => {
    it('renders CustomAutocomplete when primary type is Other', () => {
      ...
      expect(
        screen.getByTestId(
          'questionItemDetailsNonSpecifiedOtherType-0-autocomplete'
        )
      ).toBeInTheDocument();
    });
    ...
    it('does not render CustomAutocomplete when primary type is not Other', () => {
      renderComponent({
        nonSpecifiedItemDamageDetails: [
          {
            primaryType: KnownLossItemType.Jewelry,
            type: '',
            repairedOrReplaced: '',
            itemDescription: ''
          }
    ...
    it('passes the correct item selected state to CustomAutocomplete', () => {
      renderComponent({
        nonSpecifiedItemDamageDetails: [
          {
            primaryType: KnownLossItemType.Other,
            type: KnownLossItemType.Electronics,
            repairedOrReplaced: '',
            itemDescription: ''
          }
        ]
      ...
      expect(mockCustomAutocomplete).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('clears item details when autocomplete is used', () => {
      ...
      expect(
        mockClearNonSpecifiedItemDetails
  ...
  describe('mobile phone', () => {
    it('renders mobile details when item type is MobilePhones', () => {
      renderComponent({
        nonSpecifiedItemDamageDetails: [
          {
            primaryType: KnownLossItemType.Other,
            type: KnownLossItemType.MobilePhones,
            repairedOrReplaced: '',
            itemDescription: ''
          }
        ]
      ...
      expect(
        screen.getByTestId('MobileDetails')
      ).toBeInTheDocument();

      expect(
        screen.getByTestId('MobileDamageDetails')
      ).toBeInTheDocument();
    });
    ...
    it('passes the correct props to mobile components', () => {
      ...
      expect(mockMobileDetails).toHaveBeenCalledWith(
        expect.objectContaining({
          index: 0,
          modelPath: 'claim.contents.nonSpecifiedItems[0]'
        })
      ...
      expect(mockMobileDamageDetails).toHaveBeenCalledWith(
        expect.objectContaining({
          index: 0,
          modelPath: 'claim.contents.nonSpecifiedItems[0]',
          specified: false
    ...
    it('does not render item description for MobilePhones', () => {
      ...
      expect(
        screen.queryByTestId(
          'itemDetailsNonSpecifiedItemDescription-0'
        )
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe('item description', () => {
    ...
    it('passes the correct model to MDTextField', () => {
      ...
      expect(mockMDTextField).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          model:
            'claim.contents.nonSpecifiedItems[0].itemDescription',
          rows: 1
        })
      );
    });
    ...
    it('uses default description translation key when override translation does not exist', () => {
      ...
      const itemDescriptionDescriptionTranslationKey =
        `nonSpecifiedItems.itemDescription.descriptions.${type}`;
      const itemDescriptionKey =
        `claim/contents:nonSpecifiedItems.itemDescription.descriptions.${type}`;

      mockT.mockImplementation((key: string) => {
        if (key === itemDescriptionKey) {
          return itemDescriptionDescriptionTranslationKey;
        }
      ...
      const questionCall = mockQuestion.mock.calls.find(
        ([props]) =>
          props.id ===
          'questionItemDetailsNonSpecifiedItemDescription-0'
      );
    ...
    it('uses white label description translation key when override exists', () => {
      mockT.mockImplementation((key: string) => {
        if (
          key ===
          'claim/contents:nonSpecifiedItems.itemDescription.descriptions.Electronics'
        ) {
    ...
    it('renders SpectaclesDamageDetails for Spectacles', () => {
      renderComponent({
        nonSpecifiedItemDamageDetails: [
          {
            primaryType: KnownLossItemType.Other,
            type: KnownLossItemType.Spectacles,
            repairedOrReplaced: '',
            itemDescription: ''
          }
        ]
      ...
      expect(
        screen.getByTestId('SpectaclesDamageDetails')
      ).toBeInTheDocument();

      expect(mockSpectaclesDamageDetails).toHaveBeenCalledWith(
        expect.objectContaining({
          index: 0,
          modelPath: 'claim.contents.nonSpecifiedItems[0]'
        })
  ...
  describe('repaired or replaced message', () => {
    it.each([
      ...
      [
        KnownLossItemType.MobilePhones,
        YES
      ],
      [
        KnownLossItemType.Spectacles,
        YES
      ]
    ])(
      'renders repaired or replaced message for %s',
      (type, repairedOrReplaced) => {
        renderComponent({
          nonSpecifiedItemDamageDetails: [
            {
              primaryType: KnownLossItemType.Other,
    ...
    it('renders jewellery valuation message for UnspecifiedJewellery', () => {
      renderComponent({
        nonSpecifiedItemDamageDetails: [
          {
            primaryType: KnownLossItemType.Other,
            type: KnownLossItemType.UnspecifiedJewellery,
            repairedOrReplaced: '',
            itemDescription: ''
          }
        ]
  ...
  describe('maximum item count message', () => {
    it('renders max allowed message for the last allowed item', () => {
      const lastIndex =
        MAX_NON_SPECIFIED_ITEM_COUNT - 1;

      renderComponent({
        index: lastIndex,
        modelPath: `claim.contents.nonSpecifiedItems[${lastIndex}]`,
        nonSpecifiedItemDamageDetails: Array.from(
          {
            length: MAX_NON_SPECIFIED_ITEM_COUNT
          },
          (_, index) => ({
            primaryType: KnownLossItemType.Other,
            type:
              index === lastIndex
                ? KnownLossItemType.Electronics
                : '',
            repairedOrReplaced: '',
            itemDescription: ''
          })
      ...
      expect(
        screen.getByTestId(
          'maxAllowedNonSpecifiedItemsMessage'
        )
      ).toBeInTheDocument();
    });
