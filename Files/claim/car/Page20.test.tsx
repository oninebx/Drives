...
const mockUpdateDefaultLiabilityOnly =
  jest.fn();
 ...
 * Translation
 *
 * Page2 calls translate(...) at module load time.
 * We do not test the translation HOC here.
 */
 * ---------------------------------------------------------
 */
 ...
 * Car selectors / state
 *
 * Selectors have their own unit tests.
 * Page2 only needs controllable selector values.
 *
...
jest.mock('~/feature/claim/car/state', () => ({
  ...
  thunks: {
    ...
    clearPreferredRepairerSelection:
      jest.fn(),
    updateDefaultLiabilityOnlyValue:
      jest.fn()
  },
...
jest.mock('~/common/components/base', () => ({
  FormMessage: ({
    ...
    <div data-testid={`form-message-${id}`} />
  ),
